...
def audio_file(tmp_path: Path) -> Path:
    p = tmp_path / "background.ogg"
    p.touch()
    return p


@pytest.fixture
def controller_with_sound(audio_file: Path, sound_mock: MagicMock) -> KivyAudioController:
    """Controller backed by a mock Kivy Sound object."""
    with _mock_sound_loader(sound_mock):
        return KivyAudioController(music_path=audio_file)
...
def test_singleton_not_reinitialised_on_second_call(audio_file: Path, sound_mock: MagicMock) -> None:
    """Second instantiation with a different path must be ignored."""
    other_path = audio_file.parent / "other.ogg"
    other_path.touch()

    with _mock_sound_loader(sound_mock) as mock_audio_mod:
        first = KivyAudioController(music_path=audio_file)
        second = KivyAudioController(music_path=other_path)
...
def test_sound_initialised_with_loop_and_volume(audio_file: Path, sound_mock: MagicMock) -> None:
    with _mock_sound_loader(sound_mock):
        ctrl = KivyAudioController(music_path=audio_file)

    assert sound_mock.loop is True
...
def test_play_does_not_restart_when_already_playing(controller_with_sound: KivyAudioController,
                                                    sound_mock: MagicMock) -> None:
...
def test_stop_calls_sound_stop(controller_with_sound: KivyAudioController,
                               sound_mock: MagicMock) -> None:
    sound_mock.state = "play"
    controller_with_sound.stop()
    sound_mock.stop.assert_called_once()


def test_stop_does_not_call_when_already_stopped(controller_with_sound: KivyAudioController,
                                                 sound_mock: MagicMock) -> None:
    sound_mock.state = "stop"
    controller_with_sound.stop()
...
def test_toggle_stops_when_playing(controller_with_sound: KivyAudioController,
                                   sound_mock: MagicMock) -> None:
    sound_mock.state = "play"
    controller_with_sound.toggle()
    sound_mock.stop.assert_called_once()
...
def test_set_volume_updates_sound_volume(controller_with_sound: KivyAudioController,
                                         sound_mock: MagicMock) -> None:
    controller_with_sound.set_volume(0.3)
    assert sound_mock.volume == pytest.approx(0.3)


def test_is_playing_reflects_sound_state(controller_with_sound: KivyAudioController,
                                          sound_mock: MagicMock) -> None:
    sound_mock.state = "play"
...
def test_missing_audio_file_gives_silent_controller(tmp_path: Path) -> None:
    missing = tmp_path / "nonexistent.ogg"
    ctrl = KivyAudioController(music_path=missing)
